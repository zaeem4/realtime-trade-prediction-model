"""realtime-evolution-strategy-train.ipynb
"""
...
files_in_basepath = [
    f"../dataset/{entry}" for entry in entries if "ETHUSDT.csv" in entry
]

df = pd.read_csv(files_in_basepath[0])
    ...
    def train(self, epoch=100, print_every=1):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % print_every == 0:
                print(
                    "iter %d. reward: %f. at %s"
                    % (i + 1, self.reward_function(self.weights), str(datetime.now()))
                )

                copy_model = copy.deepcopy(agent.model)

                with open("../models/model-eth-m-1per-10.pkl", "wb") as fopen:
...
class Agent:
    ...
    def _initiate(self):
        ...
        print(f"printing _scaled_capital: {self._scaled_capital}")
    ...
    def get_reward(self, weights):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and starting_money >= self.trend[t]:
                with_transection_fee = self.trend[t] + (self.trend[t] * 0.01)

                inventory.append(with_transection_fee)
                starting_money -= with_transection_fee
                ...
                invest = ((with_transection_fee - bought_price) / bought_price) * 100
...
for no, stock in enumerate(files_in_basepath):
    print("training stock %s" % (stock))

    df = pd.read_csv(stock, skiprows=[i for i in range(0, 525520) if i % 10 != 0])
    df = df.dropna()
    ...
    agent.fit(iterations=400, checkpoint=10)

"""# Saving dataset"""

copy_model = copy.deepcopy(agent.model)

with open("../models/model-eth-f-1per-10.pkl", "wb") as fopen:
    pickle.dump(copy_model, fopen)
fopen.close()
