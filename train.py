    ...
    def train(self, epoch=100, print_every=1):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % print_every == 0:
                print(
                    "iter %d. reward: %f at %s" % (i + 1, self.reward_function(self.weights), str(datetime.now()))
                )
                
                copy_model = copy.deepcopy(agent.model)

                with open("../models/model-gold-m-07.pkl", "wb") as fopen:
...
with open("../models/model-gold-f-07.pkl", "wb") as fopen:
    pickle.dump(copy_model, fopen)
fopen.close()
